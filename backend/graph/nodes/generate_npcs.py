...
from graph.utils import clamp, validate_and_collect
from models.schemas import NPC, Relationship
...
def _clamp_positions(npcs: list[dict]) -> list[dict]:
    ...
    for npc in npcs:
        x = clamp(int(npc.get("x", 0)), 0, MAX_X)
        y = clamp(int(npc.get("y", 0)), 0, MAX_Y)
...
async def generate_npcs(state: SimState) -> dict:
    ...
    data = await invoke_llm_json(prompt, max_tokens=8192)

    # Validate LLM output through Pydantic models.
    npcs = validate_and_collect(data.get("npcs", []), NPC)
    relationships = validate_and_collect(data.get("relationships", []), Relationship)
