...
export function PolicyInput({ onSimulate }: PolicyInputProps) {
  ...
  return (
    <div className="w-full max-w-2xl space-y-4" data-testid="policy-input">
      ...
      <div className="rpg-panel p-4 space-y-4">
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            <label className="text-[10px] font-pixel text-white">
              AGENT POPULATION
            </label>
            <span className="text-[10px] font-pixel text-white">{numNpcs} agents</span>
          </div>
          <input
            type="range"
            min="5"
        ...
        <div className="space-y-2">
          <div className="flex justify-between items-center">
            ...
            <span className="text-[10px] font-pixel text-white">{numRounds} rounds</span>
          </div>
          <input
            type="range"
            min="3"
      ...
      <div className="relative">
        ...
        />
        <span className="absolute right-3 bottom-3 text-[10px] font-pixel text-[#555555]">
          OBJECTIVE
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-2" data-testid="preset-buttons">
        {POLICY_PRESETS.map((preset) => (
          <button
            ...
            className="rpg-panel px-4 py-2 text-[10px] font-pixel text-white transition-colors duration-150 hover:border-white hover:text-white active:translate-y-px"
          >
            {preset.label}
          </button>
        ))}
      ...
      <div className="flex items-center gap-3">
        ...
        <label
          ...
          className={`rpg-panel px-4 py-2 text-[10px] font-pixel cursor-pointer transition-colors duration-150 hover:border-white hover:text-white active:translate-y-px ${extracting ? "opacity-50 cursor-wait" : "text-white"}`}
        >
          {extracting ? "Extracting..." : "↑ Upload File"}
        </label>
        {fileName && !extracting && (
          <span className="text-[10px] font-pixel text-[#888888] truncate max-w-xs">
            {fileName}
          </span>
        )}
        <span className="text-[10px] font-pixel text-[#555555] ml-auto">
          PDF · MD · Image
        </span>
      </div>
      ...
      <button
        ...
        className="rpg-panel w-full px-6 py-4 text-sm font-pixel font-bold text-white transition-all duration-150 hover:bg-[#1a1a1a] hover:border-white hover:shadow-[0_0_12px_rgba(255,255,255,0.2)] disabled:opacity-30 disabled:cursor-not-allowed active:translate-y-px"
      >
        {">> Run Simulation <<"}
      </button>
    </div>
