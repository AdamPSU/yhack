...
const DIR_COL_OFFSET: Record<Direction, number> = {
  west: 0, south: 1, north: 2, east: 3,
};

const TILESET_COLS = 27;

/** Get a single frame tile ID */
function tileId(characterType: CharacterType, dir: Direction, row: 0 | 1 | 2): number {
...
export function getIdleFrame(characterType: CharacterType, dir: Direction): number {
  return tileId(characterType, dir, 0);
}

export function getWalkFrames(characterType: CharacterType, dir: Direction): [number, number] {
  return [tileId(characterType, dir, 1), tileId(characterType, dir, 2)];
}

export function getAnimKey(characterType: CharacterType, dir: Direction): string {
...
export const ALL_CHARACTERS: CharacterType[] = [
  "young_boy", "adult_woman", "old_man",
  "construction_worker", "adult_man", "young_girl",
];
